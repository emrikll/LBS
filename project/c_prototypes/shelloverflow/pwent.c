...
mypwent *mygetpwnam(char *name) {
	...
	static mypwent ent = { pwname, 0, passwd, passwd_salt, 0, 0, 0 };
	...
	while (fgets(buffer, sizeof(buffer), file) != NULL) {
		if (sscanf(buffer, "%[^:]:%d:%[^:]:%[^:]:%d:%d:%d", ent.pwname, &ent.uid,
				ent.passwd, ent.passwd_salt, &ent.pwfailed, &ent.pwage, &ent.locked) != 7){
					break;
				}
...
int mysetpwent(char *name, mypwent *pw) {
	...
	if ((newfile = fopen(MYPWENT_TMP_FILENAME, "wb")) == NULL) {
		fclose(file);
		printf("Could not open the temp database file \n");
		return -1;
	}
	...
	while (fgets(buffer, sizeof(buffer), file) != NULL) {
		if (sscanf(buffer, "%[^:]", pwname) != 1) {
			printf("Could not find the entry in database file \n");
			status = -1;
			break;
		}
		...
		if (strcmp(pwname, name) == 0) {
			if (snprintf(buffer, sizeof(buffer), "%s:%d:%s:%s:%d:%d:%d\n",
					pw->pwname, pw->uid, pw->passwd, pw->passwd_salt,
					pw->pwfailed, pw->pwage, pw->locked) >= sizeof(buffer)) {
	...
	if (status == 0) {
		if (rename(MYPWENT_TMP_FILENAME, MYPWENT_FILENAME) != 0) {
			status = -1;
			printf("Could not rename database file \n");
		}
	} else
